...
class GraphCut(object):
    ...
    def __init__(self, _iteration, _image, _processed):
        ...
        print("Input Image ->Total Energy: {}".format(self.calculate_total_energy()))
    ...
    def calculate_total_energy(self):
        ...
        for i in range(self.image.shape[0]):
            for j in range(self.image.shape[1]):
                temp=self.Data_Value(self.image[i][j], self.image_work, i, j)
                E_data = E_data + temp
                data[i][j]=temp
        E_smooth = 0
        for i in range(self.image.shape[0]):
            for j in range(self.image.shape[1]):
                n = self.give_neighborhood(self.image_work, i, j)
                temp= sum([self.Value(v, self.image_work[i][j]) for v in n])
                E_smooth += temp
                data[i][j]=temp
				
        self.arr_to_image(data, "totalEnergy_Main.png") 

        return E_data + E_smooth
		
    def arr_to_image(self,a, fname):
