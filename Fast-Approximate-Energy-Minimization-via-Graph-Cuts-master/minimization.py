...
def calculate_total_energy(img_orig, img_work,img_energy):
    '''Calculates Energy of image.
       img: is input array'''
    ...
    for i in range(len(img_orig)):
        for j in range(len(img_orig[0])):
            data = D_p(img_orig[i][j], img_work, j, i)
            E_data += data
            img_energy[i][j]=data
    ...
    for i in range(len(img_orig)):
        for j in range(len(img_orig[0])):
            ns = give_neighbours(img_work, j, i)
            data=sum([V_p_q(v, img_work[i][j]) for v in ns])
            E_smooth += data
            img_energy[i][j]+=data
			
    arr_to_image(img_energy, "totalEnergy_Minimization.png") 
    return E_data + E_smooth
...
def main():
    ...
    img_energy=image_to_array(img_name)
    ...
    print("#Energy input image:", calculate_total_energy(img_orig, img_work,img_energy))
